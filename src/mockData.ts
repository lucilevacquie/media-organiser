...
const createAudioItem = () => {
  ...
  const audioItem = {
    ...
    size,
    duration,
    path: `D:/media/audio/${genre.toLowerCase()}/${artist.toLowerCase()}/${artist.replaceAll(
      " ",
      "-"
    )}_${title.replaceAll(" ", "-")}.${fileType}`,
    comment,
    img,
  };
  return audioItem;
...
export interface DocItem extends Item {
  name: string;
}
...
export interface DataObject {
  [key: string]: AudioItem | ImageItem | DocItem | VideoItem;
}
