...
const Dashboard = () => {
  ...
  //From provider
  const { dataList, categories, playlists, createCategory, createPlaylist } =
    useThemeContext();
  ...
  const onCreateCategoryName = (event: React.ChangeEvent<HTMLFormElement>) => {
    event.preventDefault();
    const fd = new FormData(event.target);
    createCategory(fd.get("name")?.toString() || "");
    setShowCreateModal(false);
  };

  const onCreatePlaylistName = (event: React.ChangeEvent<HTMLFormElement>) => {
    event.preventDefault();
    const fd = new FormData(event.target);
    createPlaylist(fd.get("name")?.toString() || "");
    setShowCreateModal(false);
  };
          ...
          {modalType === "category" ? (
            <>
              ...
              >
                ...
                <button
                  type="submit"
                  className="mt-4 w-full py-2 px-4 text-center bg-gradient-to-r from-lightBlue to-pink rounded-xl text-white font-semibold"
                >
                  Create
                </button>
              </form>
        ...
        <div className="mt-8">
          ...
          <div>
            {categories.length === 0 ? (
              <button
                onClick={() => onClickCreateCategory()}
                className="hidden mt-4 button font-bold md:block"
              >
                + Create category
              </button>
            ) : (
        ...
        <div className="mt-8">
          ...
          <div>
            {playlists.length === 0 ? (
              <button
                onClick={() => onClickCreatePlaylist()}
                className="hidden mt-4 button md:block"
              >
                + Create playlist
              </button>
            ) : (
              <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-4">
                {Object.values<Playlist>(playlists).map((playlist) => (
                  <PlaylistCard
                    key={playlist.id}
                    id={playlist.id}
                    name={playlist.name}
